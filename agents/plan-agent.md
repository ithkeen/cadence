...
- **按功能维度切，不按函数 / 文件 / 行数 / 时长切**：一个 step 对应 SPEC「做什么」里**一条独立的功能**（一个 API endpoint 一个 step、一个完整的用户操作流程一个 step），不要切到"一个 helper 函数"或"一个文件"这种粒度。
...
- **视觉关联的一组 UI 合并为一个 step**：同产品族、共享导航、用户在其间跳转、需要保持视觉一致性的页面 / 组件，应放进**同一个 frontend step**，由同一个 executor 一次铺完，视觉语言（字体 / 配色 / 间距 / 组件壳）天然一致——切开多个 step 会导致每个 step 各自 declared direction，跑出多套风格。
  - 单个 frontend step 可包含多个紧密关联的 UI，但**不超过 5 个**（避免 executor 上下文过载）；超过 5 个则按产品族 / 用户旅程拆成多 step
  - **跨产品族的 UI 切开**：如"面向读者的博客前台" vs "面向作者的后台管理"是两个 step，本就该有不同视觉语言
  - 此规则仅约束 `kind: frontend`；后端 / CLI step 仍按"一条独立功能一个 step"切
