...
- 测试框架与运行命令
- 不做什么：<摘自 pai「全局范围外」+ may「实施边界 / 决策清单」约束>

## 接口与数据
- <本 phase 涉及的接口形态、数据模型、函数 / 方法契约，摘自 may 对应 REQ 的设计段>

## 错误与边界
- <失败语义、边界情形，摘自 pai 对应 REQ 的「边界与异常」和 may 对应 REQ 的「错误处理」>
...
may 缺「需求点」/「需求实现映射」/「验收标准映射」/「技术栈」任一关键段，或某个 `REQ` 没有 `AC`，或 `AC` 全是无法机器验证的描述，或 `REQ` 与全局范围外 / 实施边界字面冲突 → **不产任何 phase 文件**，返回一段简报：指出缺/冲突的是哪段（附 may 原文片段）+ 建议回 /cadence:may 补。不替用户改 may，不写 `.draft`。
