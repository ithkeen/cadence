...
**禁止**自行装框架（`npm i -D jest` / `pip install pytest` 等）；**禁止**静默跳过 TDD。装框架是 `/cadence:design` 阶段决策，不该塞进单 task。
...
- **DESIGN.md 测试框架选型 > 本 skill 通用建议**：用 jest / vitest / pytest / unittest 以 DESIGN.md / PROJECT.md 写明的为准
