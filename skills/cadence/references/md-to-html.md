...
5. **组装 HTML**：按 `composition.md §3` 固定流水线，复用 `template.html` 的 `<head>` 和外层 `<div class="page">` 骨架填入新内容；末尾保留模板内的 mermaid lazy `<script>`
...
- 原文 `![alt](src)` 或 `<img>` **原样保留**，路径不改写（相对路径 / 绝对 URL / data URI 都保留原值）
- alt 文本必填：原文有 alt 用原文；原文为空则用图片紧邻的标题、图注或上文 1 句话兜底
- **不主动新增图片**、不替换图片源、不删除图片
- 展示规格按 `components.md` 中的图片组件；若无对应组件，直接 `<img>` 裸放，由 `template.html` 的全局 CSS 兜底
...
- **路径契约**：只读三件套 + 调用方在 prompt 中传入的输入 md；只写调用方在 prompt 中传入的输出文件；标题、卷号、日期等可选参数由调用方传入。
