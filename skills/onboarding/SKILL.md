...
**图表优先**：每个分析维度优先用 Mermaid 图表展示，图表比文字更重要。参考下面的示例输出，看看图表应该长什么样。

**图表可读性优先**：Mermaid 是结构索引，不是全量清单。单张图控制在 10 个节点、12 条边以内；目录图只画顶层目录和 1-2 条关键路径，细节放到表格或列表。超过这个规模时拆成多张图，或改用表格，不要把所有文件、接口、模块压进一张图。

**Mermaid 语法安全**：节点 ID 只用短 ASCII 标识（如 `root`、`api`、`svc`），展示文本放在双引号标签里（如 `api["api: HTTP handlers"]`）。包含 `/`、`:`、`()、{}`、`[]`、逗号、空格、代码片段或中文的文本都不要直接当节点 ID。无法确认 Mermaid 语法时，改用表格或列表。
...
按以下三个层次依次展开，每个层次优先用 Mermaid 图表展示。
...
用 Mermaid `graph` 展示目录树（只展示关键路径），如果有明显的核心抽象关系，用 `classDiagram` 或 `graph` 展示。目录和抽象图都必须是摘要图，不能变成文件名横排清单。
...
3. **输出**：按"全局视角 → 骨架分析 → 数据流分析"三层结构组织文档，每层优先用 Mermaid 图表展示；复杂内容先保可读性
...
```mermaid
graph LR
  gateway["API Gateway"] --> orderSvc["订单服务"]
  userSvc["用户服务"] --> orderSvc
  db["PostgreSQL"] --> orderSvc
  orderSvc --> inventory["库存服务"]
  orderSvc --> events["Kafka: OrderEvents"]
```
...
**图表优先**：如果你发现自己写了很多文字但没有画图，立即停下来先画图。参考下面的示例输出。图表必须是可读摘要，不能牺牲可读性追求节点数量。
