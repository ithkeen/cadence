---
description: 读取 pai 产出的 REQ/AC 需求 md，只做逐需求点技术设计，产出 may-<主题>.md
...
需求已由 `/cadence:pai` 锁定。本命令**不重做需求分析**，只回答一个问题：每个 `REQ` 技术上怎么实现？——技术栈、模块怎么拆、接口和数据怎么定、函数 / 方法契约是什么、每条 `AC` 怎么验证，全部钉到下游拿着文档就能直接做实施计划、不必返工。
...
Read `<pai-path>` 全文，作为**既定需求**通读（每个 `REQ` 要做什么、给谁、每条 `AC` 怎么验收）——不重构、不改写，落档时原样保留。pai 正文即结论；`.cadence/research/` 是过程材料，此处不全量读，按需在阶段 2 定点取用。
