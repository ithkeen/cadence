...
- 非空 → **报错退出**：
  ...
  > ❌ 当前 cycle `<CURRENT 内容>` 尚未结束。
  > spec 必须串行，请手动处理：
  > - 想保留这个 cycle 的产物 → 自行整理后再开新 spec
  > - 想放弃 → `rm -rf .cadence/<cycle-dir>` 并清空 `.cadence/CURRENT`
  > 处理完再重新 `/cadence:spec`。
...
### A-5.1 定 slug + 建目录 + 写 CURRENT（原子动作）
...
- **紧接着** Write `.cadence/CURRENT` 内容为 `cycle-<slug>`。**与 mkdir 绑成原子动作**——目的是用户中途退出时 CURRENT 仍占着，便于后续手动 `rm -rf` 清理孤儿目录
...
> 进入阶段 B 时，需求段已写入 SPEC.md，CURRENT 已占用 cycle 目录。下面的步骤不再向用户做"是否进入设计"的确认。
...
1. **技术选型**：语言 / 框架 / 关键依赖 / **测试框架与运行命令**。**测试框架是 task-executor 走 TDD 的前提**——按下面分支处理，不能省：
...
5. **错误处理 / 失败语义**：对外接口 / 关键流程在失败时的形态——错误返回结构、状态码 / 错误码命名、可重试性、降级策略。task-executor 走 TDD 时第一批写的就是错误用例，这里敲定能少改
6. **关键流程**：主路径 + 重要分支的步骤
...
> - `rm -rf .cadence/<cycle-dir>` 并清空 `.cadence/CURRENT`
...
1. **先定 slug + 建目录 + 写 CURRENT**（提前到此处执行，不等 A-5）：
...
**唯一判断**：如果这个话题现在不调研，进入 `/cadence:run` 时会因为 SPEC.md 设计段信息缺口而**写不出能跑的代码**吗？

具体地，问自己：run agent 要从 SPEC.md 拿到可直接动手的信息，下面这些项是否任何一项是基于"模糊印象"、"应该是这样"得来的？只要有一项是 → **触发**。
...
判断的锚点**不是"我是否懂"，而是"run agent 真的能从 SPEC.md 拿到足够信息直接动手吗"**。模型对"自己懂"的判断在长对话里会钝化——前一轮调研产出的上下文会让你**误以为**懂了新出现的相关技术。

想说"我大概懂"时强制做这个测试：列出 run 阶段调用它需要的所有具体信息（接口路径、鉴权头、请求体 schema、响应 schema、错误码、超时/重试、版本约束……），逐项问"**我现在能把这一项精确写进 SPEC.md 吗？**"。只要有一项答不出 / 要靠猜 / 凭印象 → 当作不熟，**触发调研**。
...
- 用户中途说"算了不做了" → 不写后续文件，礼貌退出。已建的 cycle 目录与已写入的 SPEC.md 草稿不主动清理（CURRENT 已同步占着），用户可手动 `rm -rf .cadence/<cycle-dir>` 并清空 `.cadence/CURRENT`
