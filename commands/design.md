...
1. **技术选型**：语言 / 框架 / 关键依赖 / **测试框架与运行命令**。0-1 模式重点问，**测试框架必问**（task-executor 走 TDD 的前提；没选定后续业务 task 会因测试基础设施缺失而直接失败）；项目档案模式默认沿用 PROJECT.md 中的栈与测试框架，只在用户提出新栈或新增测试框架时才聊。
...
## 技术栈
- 语言 / 框架 / 关键依赖（带版本）
- 测试框架与运行命令（例如 `jest` + `npm test`、`pytest` + `pytest`、`go test ./...`）；项目档案模式下若 PROJECT.md 已记录则一行"沿用 PROJECT.md"即可
