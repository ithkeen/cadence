...
Read `.cadence/<cycle-dir>/SPEC.md`：
- 缺失 → 报错退出：`❌ 当前 cycle <cycle-dir> 缺少 SPEC.md。请回 /cadence:spec 完成需求与设计。`
- 存在 → 进 Step 3
...
| 不存在 | 存在 | **异常报错退出**：`RUN-STATE.md 存在但 PLAN.md 缺失，请人工排查。` |
...
按粒度标准 + SPEC.md 设计段的模块边界，把设计段拆成可独立执行的 task 清单。
...
- id: T1
  ...
  acceptance:
    - <客观可判定标准 1>
    - <客观可判定标准 2>
  context_hint: 参考 SPEC.md 设计段的「xxx」章节
```
...
- `context_hint`：指向 SPEC.md 设计段的相关章节名

**严禁**字段：`files`（不写死路径，让 executor 根据 PROJECT/SPEC 定位）、`area`（不预设 frontend/backend 等角色，executor 通用型）

#### Task 粒度标准

每个 task 必须满足：
1. **可独立交付**：完成后有可验收的小成果
2. **上下文可承载**：子 agent 读 task + SPEC.md 设计段指定章节就能开工
3. **验收明确**：客观可判定
4. **粗略 3h 内、改动 ≤30 文件**
...
- `prompt`：给 cycle 路径与 PLAN.md 路径，让 reviewer **自行 Read** SPEC/PROJECT/PLAN，输出结构化 JSON。**不要把这些文件全文塞进 prompt**
...
- 必读：`<cycle_dir>/SPEC.md`、`.cadence/PROJECT.md`（如存在）
...
- acceptance:
  - ...
- context_hint: <SPEC.md 设计段章节名>

[必读上下文]
- .cadence/<cycle-dir>/SPEC.md（按 context_hint 定位设计段章节）
...
- 整个 cycle 想放弃 → 提示用户手动 `rm -rf .cadence/<cycle-dir>` 并清空 `.cadence/CURRENT`
